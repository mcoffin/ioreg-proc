...
pub fn setup(reload: u32) {
    use self::reg::systick::csr::Clksource;
    get_reg().csr.update()
        .set_enable(false)
        .set_tickint(false)
        .set_clksource(Clksource::CPU);
...
pub fn ten_ms() -> Option<u32> {
  let calib = get_reg().calib.get().tenms();
  match calib {
    0 => None,
    val => Some(val)
  }
