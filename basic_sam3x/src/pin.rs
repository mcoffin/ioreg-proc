...
use crate::hal::pin;
use crate::hal::pin::{Gpio, GpioConf};
...
impl pin::Gpio for Pin {
    fn set_level(&self, level: pin::GpioLevel) {
        use self::pin::GpioLevel;
        let pio = self.controller.to_reg();
        match level {
            GpioLevel::Low => {
                pio.codr.update().set_cod(self.pin as usize, true);
            },
            GpioLevel::High => {
                pio.sodr.update().set_sod(self.pin as usize, true);
            },
        }
    }
